...
abstract class REST_Controller extends MX_Controller
{
    ...
    */
	...
	/** Function written by Muhammad Ali Khan for the purpose of logging the response to the requests already logged in the system via hooks, post_controller_hook, if id doesn't exists then it will create a new log record in the table **/
	...
	protected function log_response($response)
	{
		...
		if($response!=null)
		{
			if($this->config->item('log_id'))
			{
				$data['response'] = $response;
			        //$status = $this->rest->db->query("UPDATE ".config_item('custom_log_table')." SET response = ".$this->rest->db->escape($response)." WHERE ".$this->config->item('custom_log_table_pk')." = ".$this->rest->db->escape($this->config->item('log_id')));
					$this->db->update($this->config->item('custom_log_table'), $data, array($this->config->item('custom_log_table_pk') => $this->config->item('log_id')));
					/*, array(
			...
			else
			{
				...
				if ($this->agent->is_referral())
				{
					...
					$referrer = $this->agent->referrer();
				$variablesforview = $this->load->_ci_cached_vars;
				$accessedurl = $this->uri->uri_string;
				$requestedmodule = $this->router->fetch_module();
				$requestedclass = $this->router->fetch_class();
				$requestedmethod = $this->router->fetch_method();
				$request['USER_FK'] = getsessiondata('user_id');
				$request['LOGGING_DATE'] = date('Y-m-d H:i:s');
				$request['user_name'] = getsessiondata('user_name');
				$request['module'] = $requestedmodule;
				$request['class'] = $requestedclass;
				$request['function'] = $requestedmethod;
				$request['url'] = $accessedurl;
				$request['agent'] = $agent;
				$request['platform'] = $this->agent->platform();
				$request['referrer'] = $referrer;
				$request['get_data'] = serialize($_GET);
				$request['post_data'] = serialize($_POST);
				$request['ipaddress'] = $this->input->ip_address();
				$request['serverdata'] = serialize($_SERVER);
				$request['variablesforviews'] = serialize($variablesforview);
				$request['response'] = $response;
				$this->db->insert($this->config->item('custom_log_table'),$request);
				$id = $this->db->insert_id();
				$this->config->set_item('log_id', $id);
			}
